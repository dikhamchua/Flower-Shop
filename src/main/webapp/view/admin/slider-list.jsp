...
<body>
    ...
    <c:url value="/admin/manage-slider" var="paginationUrl">
    ...
    <div class="dashboard-main-body">
        ...
        <div class="card mb-24">
            <div class="card-body p-24">
                <form action="${pageContext.request.contextPath}/admin/manage-slider" method="GET">
                    <div class="row g-3">
                        <div class="col-md-4">
                            <input type="text" class="form-control" name="search" placeholder="Search by caption" 
                                   value="${param.search}">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            <a href="${pageContext.request.contextPath}/admin/manage-slider?action=add" class="btn btn-success w-100">
                                Add New Slider
                            </a>
                        </div>
                    </div>
                        ...
                        <tbody>
                            <c:forEach var="slider" items="${sliders}" varStatus="loop">
                                <tr>
                                    <td>${loop.index + 1}</td>
                                    <td>
                                        <img src="${slider.imageUrl}" alt="${slider.caption}" class="slider-image">
                                    </td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${slider.caption.length() > 30}">
                                                ${slider.caption.substring(0, 30)}...
                                            </c:when>
                                            <c:otherwise>
                                                ${slider.caption}
                                            </c:otherwise>
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${slider.link.length() > 30}">
                                                <a href="${slider.link}" target="_blank" title="${slider.link}">
...
function confirmDeactivate(sliderId) {
    if (confirm('Are you sure you want to deactivate this slider?')) {
        window.location.href = '${pageContext.request.contextPath}/admin/manage-slider?action=deactivate&id=' + sliderId;
    }
}

function confirmActivate(sliderId) {
    if (confirm('Are you sure you want to activate this slider?')) {
        window.location.href = '${pageContext.request.contextPath}/admin/manage-slider?action=activate&id=' + sliderId;
    }
}
</script> 