
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<main class="dashboard-main">
    <div class="navbar-header">
        <div class="row align-items-center justify-content-between">
            <div class="col-auto">
                <div class="d-flex flex-wrap align-items-center gap-4">
                    <button type="button" class="sidebar-toggle">
                        <iconify-icon icon="heroicons:bars-3-solid" class="icon text-2xl non-active"></iconify-icon>
                        <iconify-icon icon="iconoir:arrow-right" class="icon text-2xl active"></iconify-icon>
                    </button>
                    <button type="button" class="sidebar-mobile-toggle">
                        <iconify-icon icon="heroicons:bars-3-solid" class="icon"></iconify-icon>
                    </button>
                </div>
            </div>
            <div class="col-auto">
                <div class="d-flex flex-wrap align-items-center gap-3">
                    <button type="button" data-theme-toggle class="w-40-px h-40-px bg-neutral-200 rounded-circle d-flex justify-content-center align-items-center"></button>
                    <div class="dropdown">
                        <button class="btn btn-outline-secondary d-flex align-items-center gap-2" type="button" data-bs-toggle="dropdown">
                            <iconify-icon icon="heroicons:user" class="icon"></iconify-icon>
                           
                            <iconify-icon icon="heroicons:chevron-down" class="icon"></iconify-icon>
                        </button>
                        <div class="dropdown-menu dropdown-menu-end shadow-sm" style="min-width: 180px;">
                            <a href="${pageContext.request.contextPath}/authen?action=logout" 
                               class="dropdown-item d-flex align-items-center gap-2 py-2">
                                <iconify-icon icon="lucide:power" class="icon"></iconify-icon>
                                Log Out
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
